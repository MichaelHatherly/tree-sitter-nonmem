...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_nonmem", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_nonmem": ["*.pyi", "py.typed"],
        "tree_sitter_nonmem.queries": ["*.scm"],
    },
    ext_package="tree_sitter_nonmem",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_nonmem/binding.c",
                "src/parser.c",
            ],
            define_macros=[
